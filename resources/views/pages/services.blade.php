...
@section('content')
    <x-page-hero eyebrow="CORE CAPABILITIES"
        ...
        cta-text="Scope your project" :cta-href="route('contact')" />
    ...
    <x-section>
        <div class="grid gap-px bg-hairline border border-hairline rounded-3xl overflow-hidden lg:grid-cols-2">
            @foreach ([
            ...
            ] as [$n, $capability, $description, $items])
                @php
                ...
                @endphp
                <article @if ($capabilityId) id="{{ $capabilityId }}" @endif
                    class="bg-background p-8 md:p-12 scroll-mt-28">
                ...
                @foreach (array_merge($technologies, $technologies) as $i => $tech)
                    ...
                    <div class="tech-marquee__item" @if ($i >= count($technologies)) aria-hidden="true" @endif>
                        <img src="{{ $imgSrc }}" alt="{{ $name }}" width="64" height="64"
                            loading="lazy" class="tech-marquee__logo">
    ...
    <x-section>
        ...
        <div class="mt-12 grid md:grid-cols-3 gap-px bg-hairline border border-hairline rounded-2xl overflow-hidden">
            ...
            ] as [$engagementTitle, $engagementDesc])
                <div class="bg-background p-8 md:p-10">
                    <h3 class="text-xl font-semibold">{{ $engagementTitle }}</h3>
                    <p class="mt-3 text-sm text-muted-foreground leading-relaxed">{{ $engagementDesc }}</p>
                </div>
            @endforeach
        </div>
    </x-section>
    ...
    {{-- <section class="border-t border-hairline bg-surface-muted">
        <div class="container-page py-200 md:py-28">
            ...
            <div class="mt-12 max-w-3xl divide-y divide-hairline border border-hairline rounded-2xl overflow-hidden bg-background">
                @foreach ([
                    ['Do you work with existing in house engineering teams?', 'Yes. Through our Embedded Senior Talent model, we integrate directly into your sprints, tooling, and ceremonies adding senior capacity without disrupting how your team already works.'],
                    ['How do you price custom software development?', 'We operate on dedicated team retainers for ongoing product work, or fixed scope discovery phases when requirements need definition first. Every engagement starts with a transparent scope conversation no surprise change orders.'],
                    ['Do you offer SLA backed maintenance post-launch?', 'Yes. Our Cloud Infrastructure & Application Services division provides SLA-backed maintenance, proactive monitoring, security patching, and performance optimization after launch.']
                ] as [$question, $answer])
                    <details class="group">
